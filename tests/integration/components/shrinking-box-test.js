...
test('it renders large size and initial background by default', function(assert) {
  ...
  assert.equal(this.$('.shrinking-box').css("background-color"), "rgb(99, 99, 99)", 'assert color is rgb(12, 89, 100)');
});
...
test('it renders small size and appropriate background after clicking', function(assert) {
  ...
  assert.equal(this.$('.shrinking-box')[0].offsetWidth, '180', 'assert width is 180px');
  assert.equal(this.$('.shrinking-box')[0].offsetHeight, '180', 'assert height is 180px');
  assert.equal(this.$('.shrinking-box').css("background-color"), "rgb(12, 89, 100)", 'assert color is rgb(12, 89, 100)');
});
...
test('it has css transition properties', function(assert) {
  this.render(hbs`{{shrinking-box bigMessage="Big Message!" smallMessage="Small Message!"}}`);
  assert.equal(this.$('.shrinking-box').css("transition"), "width 0.5s ease 0s, height 0.5s ease 0s, background-color 0.5s ease 0s", 'assert shrinking box has transition properties');
});
